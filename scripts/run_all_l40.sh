#!/usr/bin/env bash
# Drive the full method matrix on the L40. One server at a time on port 8000/30000.
set -uo pipefail
...
# Thunder Compute vGPU: multi-process EngineCore crashes the runtime; run in-process.
...
wait_ready() { # port log
  ...
  for _ in $(seq 1 180); do
    ...
    if [ "$code" = "200" ]; then
      # CUDA graphs / compile can still be in flight after the first 200.
      sleep 25
      code=$(curl -s -o /dev/null -w "%{http_code}" "http://127.0.0.1:${port}/v1/models" || echo 000)
...
# vLLM matrix (EAGLE-3 K=3 already benched before this script)
...
# SGLang matrix
run_cfg sglang configs/sglang_ar.yaml 30000

#!/usr/bin/env bash
# SGLang EAGLE-3 / adaptive only (ninja is now on PATH).
set -uo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"
mkdir -p results
FAIL_RE="Engine core initialization failed|Received sigquit|Not enough memory|OutOfMemoryError|Thunder Compute runtime has crashed|gated repo|403 Client Error|Cannot access gated|awaiting a review|No such file or directory: 'ninja'"
...
stop_servers
echo "SGLANG_EAGLE_RETRY_COMPLETE"
